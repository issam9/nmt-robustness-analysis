...
def process_files(src_lang, target_langs, data_path, seed=123):
    for lang in target_langs:
        for error_type in ["prep", "nounnum", "article"]:
            # Get all files matching the error type for the language pair
            files = glob.glob(f'{data_path}/{src_lang}-{lang}/train.{error_type}.*')
            save_path = f'./data/finetuning-subset/{src_lang}/{lang}'
            ...
            for file in files:
                ...
                with open(Path(save_path).joinpath(f'train/segments.clean.{file.split(".")[-3]}.{src_lang}'), 'w') as f:
                ...
                with open(Path(save_path).joinpath(f'train/segments.clean.{file.split(".")[-3]}.{lang}'), 'w') as f:
...
def pkl_to_txt(src_lang, target_langs):
    for lang in target_langs:
        for split in ['train_finetuning', 'dev', 'test']:
            save_path = f'./data/finetuning-subset/{src_lang}/{lang}/{split.split("_")[0]}'
            Path(save_path).mkdir(exist_ok=True, parents=True)

            for error in ['prep', 'nounnum', 'article']:
                # Load the pickle file for the given split and error type
                pkl_file_path = f'./data/grammar-noise/{src_lang}-{lang}/{split}.{error}.{src_lang}.pkl'
                df = pd.read_pickle(pkl_file_path)

                # Detokenize the sentences
                sentences = df.tokenized_line.tolist()
                detokenized_sentences = [TreebankWordDetokenizer().detokenize(sentence) for sentence in sentences]

                # Save detokenized sentences to the segment file
                segment_file_path = Path(save_path).joinpath(f'segments.{error}.{src_lang}')
                with open(segment_file_path, 'w') as f:
                    for sentence in detokenized_sentences:
                        f.write(sentence + '\n')
                
                print(f"{lang}---{split}---{error}---{len(sentences)} sentences saved to {segment_file_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target-langs', type=str, nargs='+', required=True, help="Target languages")
    ...
    process_files(args.src_lang, args.target_langs, args.data_path, args.seed)

    pkl_to_txt(args.src_lang, args.target_langs)
