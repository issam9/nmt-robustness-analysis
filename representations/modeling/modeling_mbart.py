...
class MBartEncoder(MBartPreTrainedModel):
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            # if we want to get masked head representations then we skip the input layer
                if head_mask is None or idx > 0:
                    states_to_add = masked_hidden_states if head_mask is not None else hidden_states
                    encoder_states = encoder_states + (states_to_add,)
            ...
            if to_drop:
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    # We want to mask only the head from previous layer, that's why we pass the original hidden_states
                    if head_mask is not None:
                        masked_layer_outputs = encoder_layer(
                            hidden_states,
                            attention_mask,
                            layer_head_mask=head_mask[idx],
                            output_attentions=output_attentions,
                        )
                        masked_hidden_states = masked_layer_outputs[0]
        ...
        if output_hidden_states:
            if head_mask is not None:
                masked_hidden_states = self.layer_norm(masked_hidden_states)
                encoder_states = encoder_states + (masked_hidden_states,)
            else:
